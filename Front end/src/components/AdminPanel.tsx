...
function buildOrderSummary(member: Member): string {
  const key      = getPackKey(member);
  const info     = PACK_INFO[key];
  const qty      = getQuantity(member);
  const soaps    = info.soapsPerBox * qty;
  const amount   = info.pricePerBox * qty;
  const mrp      = info.mrpPerBox ? info.mrpPerBox * qty : null;
  const savings  = mrp ? mrp - amount : 0;

  let desc = `📦 ${info.label}`;
  if (key !== "RED_SANDAL") {
    desc += ` | Quantity: ${qty} = ${soaps} soaps total`;
    if (mrp) desc += ` | MRP ₹${mrp.toLocaleString()} → ₹${amount.toLocaleString()} (Save ₹${savings.toLocaleString()})`;
    else     desc += ` | ₹${amount.toLocaleString()}`;
  } else {
    desc += ` | ${qty} Kit${qty > 1 ? "s" : ""} | 14 Ayurvedic Products | ₹${amount.toLocaleString()}`;
  }
  return desc;
}
